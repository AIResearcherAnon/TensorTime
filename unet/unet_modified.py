...
class UNetModel(nn.Module):
    ...
    def forward(self, interpolants, x, y=None):
        ...
        emb = torch.zeros([x.shape[0], self.time_embed_dim]).type_as(x)

        x = torch.cat((x, interpolants), dim=1)
...
class UNetModelWrapper(UNetModel):
    ...
    def forward(self, interpolants, x, y=None, *args, **kwargs):
        return super().forward(interpolants, x, y=y)
