...
from unet.unet_modified import UNetModelWrapper

from adaptive_multidimensional_path import AdaptiveMultidimensionalPath

from unet_for_p.unet_model import UNet
...
def main():
    ...
    time = 'lpft'
    scale = '0.1'
    g_training_step = 400000
    p_training_step = 190000
    ...
    batch_size = 500
    num_iteration = 100
    sample_saving_path = f'./sample_path_optimize/{time}/{scale}/{spatial_dimension}/{M}/{N}'
    ...
    checkpoint = torch.load(f'./models/{time}/{scale}/model_cifar10_weights_step_{g_training_step}.pt')
    ...
    checkpoint = torch.load(f'./models/{time}/{scale}/path_optimization/{M}/{spatial_dimension}/model_cifar10_weights_step_{p_training_step}.pt')
    state_dict = checkpoint['ema_model']
